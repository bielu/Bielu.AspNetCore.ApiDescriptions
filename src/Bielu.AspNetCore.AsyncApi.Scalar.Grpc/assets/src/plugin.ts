...
export const GRPC_CONSOLE_TAG = 'bielu-grpc-console'

/** The Scalar plugin's identifier, used to detect (and de-duplicate) an existing registration. */
export const GRPC_PLUGIN_NAME = 'bielu-grpc'

/** How the gRPC console registers with Scalar (plugin name, element tag, sidebar label). */
export const GRPC_PLUGIN_SPEC = {
  pluginName: GRPC_PLUGIN_NAME,
  elementTag: GRPC_CONSOLE_TAG,
  sidebarLabel: 'gRPC',
} as const
...
/**
 * Creates the gRPC console plugin for the Scalar API Reference.
 *
 ...
 */
export const createGrpcPlugin = (documents?: GrpcDocumentRef[]): ApiReferencePlugin =>
  createConsolePlugin(GRPC_PLUGIN_SPEC, documents)
