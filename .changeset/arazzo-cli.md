...
Also extracted `Bielu.Cli.Shared`, the console-CLI infrastructure (`ConsoleCliLogger`, `CliArgumentReader`, `CliFileResolver`, and the `validate`/`diff` report renderers) that `dotnet asyncapi` and `dotnet arazzo` would otherwise have duplicated. `Bielu.AspNetCore.AsyncApi.Cli` now builds on the same shared package; its behavior and tests are unchanged.
