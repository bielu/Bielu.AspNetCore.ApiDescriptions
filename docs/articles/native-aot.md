...
- **Publish warnings from `ByteBard.AsyncAPI`**: the dependency is not trim/AOT-annotated, so an AOT publish reports `IL2104`/`IL3053` from that assembly. They come from outside this project and do not affect the generated document — the `aot-verification` CI job asserts the AOT build serves a byte-for-byte equivalent document to the reflection-based build.
