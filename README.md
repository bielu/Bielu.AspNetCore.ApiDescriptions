...
This library generates a standard AsyncAPI document at `/asyncapi/{documentName}.json` (default: `/asyncapi/v1.json`), so you can render it with any AsyncAPI-compatible UI.

### Recommended: Scalar

We recommend [Scalar](https://scalar.com/) for viewing AsyncAPI documentation. It now renders AsyncAPI documents — channels, operations, messages, and component schemas — alongside your OpenAPI references in a single, modern UI. Because Scalar simply consumes the generated document, you can keep using the same `MapAsyncApi()` endpoint this library exposes.

> ⚠️ **Note:** Scalar's AsyncAPI support is still evolving and not every part of the specification is rendered yet. Track progress at [scalar/scalar#7080](https://github.com/scalar/scalar/issues/7080).

#### ASP.NET Core integration

Install the Scalar package and point it at the document generated by this library:
...
builder.Services.AddAsyncApi(options =>
{
    options.WithDescription("My event-driven API");
});

var app = builder.Build();

// Expose the generated AsyncAPI document at /asyncapi/{documentName}.json
app.MapAsyncApi();

// Render it with Scalar
app.MapScalarApiReference(options =>
{
    // documentName "v1" maps to /asyncapi/v1.json
    options.AddAsyncApiDocument("v1", "My event-driven API", "/asyncapi/v1.json");
});

app.Run();
```

Scalar can mix AsyncAPI and OpenAPI documents in the same reference:

```csharp
app.MapScalarApiReference(options =>
{
    options
        .AddDocument("openapi", "REST API")               // your OpenAPI document
        .AddAsyncApiDocument("v1", "Event Stream", "/asyncapi/v1.json");
});
```

#### Standalone (CDN) embed

If you host the UI yourself, point Scalar's standalone bundle at the generated document URL:
...
##### `merge`
Merge multiple AsyncAPI documents into one:
...
##### `validate`
Validate AsyncAPI documents against the spec:
...
##### `diff`
Compare two AsyncAPI documents for changes:
